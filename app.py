...
import urllib.parse 
import json
import os # สำหรับตรวจสอบไฟล์

# กำหนดชื่อไฟล์สำหรับเก็บข้อมูลผู้ใช้
USER_DATA_FILE = "user_data.json"

# ----------------------------------------------------------------------------------
# 💾 ฟังก์ชันจัดการข้อมูลผู้ใช้ (Load/Save)
# ----------------------------------------------------------------------------------

def load_user_data():
    """โหลดข้อมูลผู้ใช้จากไฟล์ JSON"""
    if not os.path.exists(USER_DATA_FILE):
        # หากไฟล์ไม่มี ให้สร้างด้วยข้อมูลเริ่มต้น
    ...
    try:
        ...
        # กรณีไฟล์ว่างเปล่าหรือมีข้อผิดพลาด
        st.error("❗ ข้อมูลผู้ใช้ในไฟล์เสีย กรุณาตรวจสอบ user_data.json")
        return {"admin": "1234", "user01": "pass"} # ส่งคืนค่าเริ่มต้นฉุกเฉิน
...
# -------------------------------
# 🔐 ระบบผู้ใช้ (จำลอง) - โหลดข้อมูลถาวร
# -------------------------------
# ❗ เปลี่ยนการตั้งค่า users จาก st.session_state เป็นการโหลดจากไฟล์ถาวร
...
if not st.session_state.logged_in:
    ...
    # ❗ โหลดข้อมูลผู้ใช้ล่าสุดจากไฟล์ JSON
    ...
    with tab1:
        ...
        if st.button("เข้าสู่ระบบ"):
            # ❗ ตรวจสอบกับข้อมูลที่โหลดมาจากไฟล์
    ...
    with tab2:
        ...
        if st.button("สมัครสมาชิก"):
            ...
            # ❗ โหลดข้อมูลล่าสุดอีกครั้งเพื่อความชัวร์ก่อนสมัคร
            users = load_user_data() 
            ...
            if new_user in users:
                ...
                # ❗ เพิ่มผู้ใช้ใหม่ลงใน Dictionary และบันทึกไฟล์
...
if filtered_restaurants:
    for r in filtered_restaurants:
        ...
        col_title = st.columns([1])[0] # เหลือแค่คอลัมน์เดียวสำหรับชื่อ
        ...
        # ❗❗ ส่วนที่แก้ไข: จัดปุ่ม Favorite และ Google Maps ให้อยู่ในบรรทัดเดียวกัน ❗❗
        col_map, col_fav_btn = st.columns([2, 1])
